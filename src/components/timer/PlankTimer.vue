<template>
  <div class="plank-timer">
    ...
    <div class="ring-wrapper">
      <div v-if="phase === 'running'" class="ring-glow"
      ...
      <svg :width="SIZE" :height="SIZE" :viewBox="`0 0 ${SIZE} ${SIZE}`">
        ...
        <!-- Second markers -->
        <line v-for="(_, i) in duration" :key="i"
          :x1="markerInner(i).x" :y1="markerInner(i).y"
          :x2="markerOuter(i).x" :y2="markerOuter(i).y"
          :stroke="i < elapsed ? ringColor : 'rgba(255,255,255,0.07)'"
          stroke-width="1.5" class="second-marker" />

        <!-- Center: Done -->
        <template v-if="isDone">
          <text :x="cx" :y="cy - 4" text-anchor="middle"
            fill="var(--green)" font-size="28">✓</text>
          <text :x="cx" :y="cy + 14" text-anchor="middle"
            fill="rgba(110,231,122,0.5)" font-size="9" letter-spacing="1">SELESAI</text>
        </template>
        ...
        <template v-else>
          <text :x="cx" :y="cy + 10" text-anchor="middle"
            ...
            class="timer-text">{{ timeLeft }}</text>
          <text :x="cx" :y="cy + 26" text-anchor="middle"
            fill="rgba(255,255,255,0.18)" font-size="10" letter-spacing="1">detik</text>
        </template>
      </svg>

      <!-- Pulse on each tick -->
      <div v-if="pulse && phase === 'running'"
        class="tick-pulse" :class="{ 'warn-pulse': isWarn }" />
    </div>
    ...
    <div class="controls">
      ...
      <button v-if="phase === 'running'" class="reset-btn" @click="resetTimer">↺</button>
    </div>
  </div>
</template>
...
const progress = computed(() => elapsed.value / props.duration)

const strokeOffset = computed(() => circumference - progress.value * circumference)
...
const liveCalories   = computed(() => calcCalories(props.weightKg, elapsed.value).toFixed(4))
const finalCalories  = computed(() => calcCalories(props.weightKg, props.duration).toFixed(2))
...
function haptic(type) {
  ...
  if (type === 'tick')     navigator.vibrate(30)           // singkat tiap detik normal
  if (type === 'warn')     navigator.vibrate([60, 40, 60]) // dua kali saat warning
  if (type === 'done')     navigator.vibrate([100, 80, 200, 80, 400]) // panjang saat selesai
}
...
async function startTimer() {
  ...
  interval = setInterval(() => {
    ...
    const warn = timeLeft.value <= 3

    // Audio + haptic berbeda untuk warn vs normal
    playSound(warn ? 'warn' : 'tick')
    haptic(warn ? 'warn' : 'tick')
...
.ring-wrapper { position: relative; width: 180px; height: 180px; }
...
.timer-text { transition: font-size 0.1s, fill 0.3s; }
...
.warn-pulse { border-color: rgba(255,107,107,0.4); }
@keyframes pulseOut {
  0%   { transform: scale(1); opacity: 0.8; }
  100% { transform: scale(1.28); opacity: 0; }
}

.calorie-live { font-size: 11px; letter-spacing: 1px; }
...
.start-btn-idle    { background: rgba(240,200,96,0.15); border: 1px solid rgba(240,200,96,0.35); color: var(--gold); }
.start-btn-running { background: rgba(240,200,96,0.07); border: 1px solid rgba(240,200,96,0.18); color: rgba(240,200,96,0.45); cursor: default; }
.start-btn-warn    { background: rgba(255,107,107,0.12); border: 1px solid rgba(255,107,107,0.3); color: var(--red); cursor: default; }

.done-badge {
  padding: 10px 18px;
  background: rgba(110,231,122,0.1); border: 1px solid rgba(110,231,122,0.2);
